...
/**
 * ComplianceFormsApi Class Doc Comment
 *
 * @category Class
 * @package  Finix
 * @author   Finix 
...
class ComplianceFormsApi
{
    ...
    public function listWithHttpInfo($compliance_forms_id)
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Finix\Model\ComplianceForm' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\Finix\Model\ComplianceForm' !== 'string') {
                            $content = json_decode($content);
                        }
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\Finix\Model\ComplianceForm', []),
    ...
    public function listRequest($compliance_forms_id)
    {
        ...
        if ($compliance_forms_id === null || (is_array($compliance_forms_id) && count($compliance_forms_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $compliance_forms_id when calling getComplianceForms'
            );
        }

        $resourcePath = '/compliance_forms/{compliance_forms_id}';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // path params
        if ($compliance_forms_id !== null) {
            $resourcePath = str_replace(
                '{' . 'compliance_forms_id' . '}',
                ObjectSerializer::toPathValue($compliance_forms_id),
                $resourcePath
            );
        }
    ...
    public function updateRequest($compliance_forms_id, $update_compliance_form_request = null)
    {
        ...
        if ($compliance_forms_id === null || (is_array($compliance_forms_id) && count($compliance_forms_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $compliance_forms_id when calling updateComplianceForms'
            );
        }
